...
class WellBores:
    def __init__(self, model:Model):
        ...
        self.nprod = self.ParameterDict[self.nprod.Name] = intParameter("Number of Production Wells", value = 2, DefaultValue=2, AllowableRange=list(range(1,201,1)), UnitType = Units.NONE, Required=True, ErrMessage = "assume default number of production wells (2)", ToolTipText="Number of (identical) production wells")
        self.ninj = self.ParameterDict[self.ninj.Name] = intParameter("Number of Injection Wells", value = 2, DefaultValue=2, AllowableRange=list(range(0,201,1)), UnitType = Units.NONE, Required=True, ErrMessage = "assume default number of injection wells (2)", ToolTipText="Number of (identical) injection wells")
        self.prodwelldiam =  self.ParameterDict[self.prodwelldiam.Name] = floatParameter("Production Well Diameter", value = 8.0, DefaultValue=8.0, Min=1, Max=30, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.INCHES, CurrentUnits = LengthUnit.INCHES, Required=True, ErrMessage = "assume default production well diameter (8 inch)", ToolTipText="Inner diameter of production wellbore (assumed constant along the wellbore) to calculate frictional pressure drop and wellbore heat transmission with Rameys model")
        self.injwelldiam = self.ParameterDict[self.injwelldiam.Name] = floatParameter("Injection Well Diameter", value = 8.0, DefaultValue=8.0, Min=1, Max=30, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.INCHES, CurrentUnits = LengthUnit.INCHES, Required=True, ErrMessage = "assume default injection well diameter (8 inch)", ToolTipText="Inner diameter of production wellbore (assumed constant along the wellbore) to calculate frictional pressure drop and wellbore heat transmission with Rameys model")
        self.rameyoptionprod = self.ParameterDict[self.rameyoptionprod.Name] = boolParameter("Ramey Production Wellbore Model", value = True, DefaultValue=True, UnitType = Units.NONE, Required=True, ErrMessage = "assume default production wellbore model (Ramey model active)", ToolTipText="Select whether to use Rameys model to estimate the geofluid temperature drop in the production wells")
        self.tempdropprod = self.ParameterDict[self.tempdropprod.Name] = floatParameter("Production Wellbore Temperature Drop", value = 5.0, DefaultValue=5.0, Min=-5, Max=50, UnitType = Units.TEMPERATURE, PreferredUnits = TemperatureUnit.CELCIUS, CurrentUnits = TemperatureUnit.CELCIUS, ErrMessage = "assume default production wellbore temperature drop (5 deg.C)", ToolTipText="Specify constant production well geofluid temperature drop in case Rameys model is disabled.")
        self.tempgaininj = self.ParameterDict[self.tempgaininj.Name] = floatParameter("Injection Wellbore Temperature Gain", value = 0.0, DefaultValue=0.0, Min=-5, Max=50, UnitType = Units.TEMPERATURE, PreferredUnits = TemperatureUnit.CELCIUS, CurrentUnits = TemperatureUnit.CELCIUS, ErrMessage = "assume default injection wellbore temperature gain (0 deg.C)", ToolTipText="Specify constant injection well geofluid temperature gain.")
        self.prodwellflowrate = self.ParameterDict[self.prodwellflowrate.Name] = floatParameter("Production Flow Rate per Well", value = 50.0, DefaultValue=50.0, Min = 1, Max=500, UnitType = Units.FLOWRATE, PreferredUnits = FlowRateUnit.KGPERSEC, CurrentUnits = FlowRateUnit.KGPERSEC, ErrMessage = "assume default flow rate per production well (50 kg/s)", ToolTipText="Geofluid flow rate per production well.")
        self.impedance = self.ParameterDict[self.impedance.Name] = floatParameter("Reservoir Impedance", value = 1000.0, DefaultValue=1000.0, Min = 1E-4, Max = 1E4, UnitType = Units.IMPEDANCE, PreferredUnits = ImpedanceUnit.GPASPERM3, CurrentUnits = ImpedanceUnit.GPASPERM3, ErrMessage = "assume default reservoir impedance (0.1 GPa*s/m^3)", ToolTipText="Reservoir resistance to flow per well-pair. For EGS-type reservoirs when the injection well is in hydraulic communication with the production well, this parameter specifies the overall pressure drop in the reservoir between injection well and production well (see docs)")
        self.wellsep = self.ParameterDict[self.wellsep.Name] = floatParameter("Well Separation", value = 1000.0, DefaultValue=1000.0, Min=10, Max=10000, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.METERS, CurrentUnits = LengthUnit.INCHES, ErrMessage = "assume default well seperation (1000 m)", ToolTipText="Well separation for built-in TOUGH2 doublet reservoir model")
        self.Tinj = self.ParameterDict[self.Tinj.Name] = floatParameter("Injection Temperature", value = 70.0, DefaultValue=70.0, Min= 0, Max=200, UnitType = Units.TEMPERATURE, PreferredUnits = TemperatureUnit.CELCIUS, CurrentUnits = TemperatureUnit.CELCIUS, Required=True, ErrMessage = "assume default injection temperature (70 deg.C)", ToolTipText="Constant geofluid injection temperature at injection wellhead.")
        self.Phydrostatic = self.ParameterDict[self.Phydrostatic.Name] = floatParameter("Reservoir Hydrostatic Pressure", value = 1E2, DefaultValue=1E2, Min=1E2, Max=1E5, UnitType = Units.PRESSURE, PreferredUnits = PressureUnit.KPASCAL, CurrentUnits = PressureUnit.KPASCAL, ErrMessage = "calculate reservoir hydrostatic pressure using built-in correlation", ToolTipText="Reservoir hydrostatic far-field pressure.  Default value is calculated with built-in modified Xie-Bloomfield-Shook equation (DOE, 2016).")
        self.ppwellhead = self.ParameterDict[self.ppwellhead.Name] = floatParameter("Production Wellhead Pressure", value = 101.3200 + 344.7, DefaultValue=101.3200 + 344.7, Min=0.0, Max=1E4, UnitType = Units.PRESSURE, PreferredUnits = PressureUnit.KPASCAL, CurrentUnits = PressureUnit.KPASCAL, ErrMessage = "using Water vapor pressure (101.3200; at 100 C) + 344.7 kPa (50 psi).", ToolTipText="Constant production wellhead pressure; Required if specifying productivity index")
        self.II = self.ParameterDict[self.II.Name] = floatParameter("Injectivity Index", value = 10.0, DefaultValue=10.0, Min=1E-2, Max=1E4, UnitType = Units.INJECTIVITY_INDEX, PreferredUnits = InjectivityIndexUnit.KGPERSECPERBAR, CurrentUnits = InjectivityIndexUnit.KGPERSECPERBAR, ErrMessage = "assume default injectivity index (10 kg/s/bar)", ToolTipText="Injectivity index defined as ratio of injection well flow rate over injection well outflow pressure drop (flowing bottom hole pressure - hydrostatic reservoir pressure).")
        self.PI = self.ParameterDict[self.PI.Name] = floatParameter("Productivity Index", value = 10.0, DefaultValue=10.0, Min=1E-2, Max=1E4, UnitType = Units.PRODUCTIVITY_INDEX, PreferredUnits = ProductivityIndexUnit.KGPERSECPERBAR, CurrentUnits = ProductivityIndexUnit.KGPERSECPERBAR, ErrMessage = "assume default productivity index (10 kg/s/bar)", ToolTipText="Productivity index defined as ratio of production well flow rate over production well inflow pressure drop (see docs)")
...
#        self.HasHorizontalSection = self.ParameterDict[self.HasHorizontalSection.Name] = boolParameter("Has Nonvertical Section", value = False, DefaultValue=False, UnitType = Units.NONE, Required=False, ErrMessage = "assume default has no horizontal section", ToolTipText="Set to true if the wells have a horizontal section (as in closed loops)")
        self.IsAGS = self.ParameterDict[self.IsAGS.Name] = boolParameter("Is AGS", value = False, DefaultValue=False, UnitType = Units.NONE, Required=False, ErrMessage = "assume default is not AGS", ToolTipText="Set to true if the model is for an Advanced Geothermal System (AGS)")
...
#MIR Does Tinj, ninj, nprod, prodwellflowrate change values?
        ...
        self.PumpingPowerProd = self.OutputParameterDict[self.PumpingPowerProd.Name] = OutputParameter(Name = "PumpingPowerProd", value=[0.0], UnitType = Units.POWER, PreferredUnits = PowerUnit.MW, CurrentUnits = PowerUnit.MW)
        self.PumpingPowerInj = self.OutputParameterDict[self.PumpingPowerInj.Name] = OutputParameter(Name = "PumpingPowerInj", value=[0.0], UnitType = Units.POWER, PreferredUnits = PowerUnit.MW, CurrentUnits = PowerUnit.MW)
        ...
        self.DPOverall = self.OutputParameterDict[self.DPOverall.Name] = OutputParameter(Name = "Total Pressure Drop", value=[0.0], UnitType = Units.PRESSURE, PreferredUnits = PressureUnit.KPASCAL, CurrentUnits = PressureUnit.KPASCAL)
        self.DPInjWell = self.OutputParameterDict[self.DPInjWell.Name] = OutputParameter(Name = "Injection Well Pressure Drop", value=[0.0], UnitType = Units.PRESSURE, PreferredUnits = PressureUnit.KPASCAL, CurrentUnits = PressureUnit.KPASCAL)
        self.DPReserv = self.OutputParameterDict[self.DPReserv.Name] = OutputParameter(Name = "Reservoir Pressure Drop", value=[0.0], UnitType = Units.PRESSURE, PreferredUnits = PressureUnit.KPASCAL, CurrentUnits = PressureUnit.KPASCAL)
        self.DPProdWell = self.OutputParameterDict[self.DPProdWell.Name] = OutputParameter(Name = "Production Well Pump Pressure Drop", value=[0.0], UnitType = Units.PRESSURE, PreferredUnits = PressureUnit.KPASCAL, CurrentUnits = PressureUnit.KPASCAL)
        self.DPBouyancy = self.OutputParameterDict[self.DPBouyancy.Name] = OutputParameter(Name = "Bouyancy Pressure Drop", value=[0.0], UnitType = Units.PRESSURE, PreferredUnits = PressureUnit.KPASCAL, CurrentUnits = PressureUnit.KPASCAL)
        ...
        self.PumpingPower = self.OutputParameterDict[self.PumpingPower.Name] = OutputParameter(Name = "Pumping Power", value=[0.0], UnitType = Units.POWER, PreferredUnits = PowerUnit.MW, CurrentUnits = PowerUnit.MW)
        ...
        if len(model.InputParameters) > 0:
            ...
            for item in self.ParameterDict.items():
                ...
                if key in model.InputParameters:
                    ...
                    if ParameterToModify.Name == "Reservoir Impedance":  #impedance: impedance per wellpair (input as GPa*s/m^3 and converted to KPa/kg/s (assuming 1000 for density; density will be corrected for later))
    ...
    def RameyCalc(self, krock:float, rhorock:float, cprock:float, welldiam:float, tv, utilfactor:float, flowrate:float, cpwater:float, Trock:float,
        ...
        framey[1:] = -np.log(1.1*(welldiam/2.0)/np.sqrt(4.*alpharock*tv[1:]*365.0*24.0*3600.0*utilfactor))-0.29
    ...
    def WellPressureDrop(self, model:Model, Taverage:float, wellflowrate:float, welldiam:float, impedancemodelused:bool, depth:float):
        #start by calculating wellbore fluid conditions [kPa], noting that most of temperature drop happens in upper section (because surrounding rock temperature is lowest in upper section)
        ...
        if Rewateraverage < 2300.0: f3 = 64./Rewater
        else:
            ...
            f3 = 1./np.power((-2*np.log10(relroughness/3.7+2.51/Rewater/np.sqrt(f3))),2.)
            f3 = 1./np.power((-2*np.log10(relroughness/3.7+2.51/Rewater/np.sqrt(f3))),2.)
            f3 = 1./np.power((-2*np.log10(relroughness/3.7+2.51/Rewater/np.sqrt(f3))),2.) #6 iterations to converge

        if impedancemodelused: DPWell = f3*(rhowater*v**2/2.)*(depth/welldiam)/1E3     ##assumed everything stays liquid throughout; /1E3 to convert from Pa to kPa
        else: DPWell = []    #it will be calculated elsewhere

        return DPWell, f3, v, rhowater

    def InjectionWellPressureDrop(self, model:Model, Taverage:float, wellflowrate:float, welldiam:float, impedancemodelused:bool, depth:float, nprod:int, ninj:int, waterloss:float):
        #start by calculating wellbore fluid conditions [kPa], noting that most of temperature drop happens in upper section (because surrounding rock temperature is lowest in upper section)
        rhowater = model.reserv.densitywater(Taverage)*np.linspace(1,1,len(self.ProducedTemperature.value))
        muwater = model.reserv.viscositywater(Taverage)*np.linspace(1,1,len(self.ProducedTemperature.value))  #replace with correlation based on Tinjaverage
        v = nprod/ninj*wellflowrate*(1.0 + waterloss)/rhowater/(math.pi/4.*welldiam**2)
        Rewater = 4.*nprod/ninj*wellflowrate*(1.0 + waterloss)/(muwater*math.pi*welldiam) #laminar or turbulent flow?
        Rewateraverage = np.average(Rewater)
        if Rewateraverage < 2300. : #laminar flow
            ...
            f1 = 1./np.power((-2*np.log10(relroughness/3.7+2.51/Rewater/np.sqrt(f1))),2.)
            f1 = 1./np.power((-2*np.log10(relroughness/3.7+2.51/Rewater/np.sqrt(f1))),2.)
            f1 = 1./np.power((-2*np.log10(relroughness/3.7+2.51/Rewater/np.sqrt(f1))),2.)  #6 iterations to converge
    ...
    def ProdPressureDropsAndPumpingPowerUsingImpedenceModel(self, f3:float, vprod:float, rhowaterinj:float, rhowaterprod:float,
                                                        rhowaterreservoir:float, depth:float, wellflowrate:float, prodwelldiam:float,
                                                        impedance:float, nprod:int, waterloss:float, pumpeff:float):
    ...
    def InjPressureDropsAndPumpingPowerUsingImpedenceModel(self, f1:float, vinj:float, rhowaterinj:float, depth:float, wellflowrate:float,
                                                          injwelldiam:float, ninj:int, waterloss:float, pumpeff:float, DPOverall):
    ...
    def ProdPressureDropAndPumpingPowerUsingIndexes(self, model:Model, usebuiltinhydrostaticpressurecorrelation:bool, productionwellpumping:bool, usebuiltinppwellheadcorrelation:bool,
                                                    Trock:float, Tsurf:float, depth:float, gradient:float, ppwellhead:float, PI: float, wellflowrate:float, f3:float, vprod:float,
                                                    prodwelldiam:float, nprod:int, pumpeff:float,  rhowaterprod:float):
        #Calculate PressureDrops and Pumping Power needed for the production well using indexes
        PumpingPower = PumpingPowerProd = DPProdWell = Pprodwellhead = ([0.0] * len(vprod))  #initialize value in case it doesn't get set.
        #reservoir hydrostatic pressure [kPa]
        if usebuiltinhydrostaticpressurecorrelation:
            CP = 4.64E-7
            CT = 9E-4/(30.796*Trock**(-0.552))
            Phydrostaticcalc = 0+1./CP*(math.exp(model.reserv.densitywater(Tsurf)*9.81*CP/1000*(depth-CT/2*gradient*depth**2))-1)
        ...
        if productionwellpumping:
            ...
            Pminimum = self.vaporpressurewater(Trock) + Pexcess #[kPa] is minimum production pump inlet pressure and minimum wellhead pressure
            if usebuiltinppwellheadcorrelation: Pprodwellhead = Pminimum   #production wellhead pressure [kPa]
            else:
            ...
            pumpdepth = depth + (Pminimum - Phydrostaticcalc + wellflowrate/PIkPa)/(f3*(rhowaterprod*vprod**2/2.)*(1/prodwelldiam)/1E3 + rhowaterprod*9.81/1E3)
    ...
    def InjPressureDropAndPumpingPowerUsingIndexes(self, model:Model, usebuiltinhydrostaticpressurecorrelation:bool, productionwellpumping:bool, usebuiltinppwellheadcorrelation:bool, usebuiltinoutletplantcorrelation:bool,
                                                   Trock:float, Tsurf:float, depth:float, gradient:float, ppwellhead:float, II:float, wellflowrate:float, f1:float, vinj:float, injwelldiam:float, nprod:int, ninj:int,
                                                   waterloss:float, pumpeff:float,  rhowaterinj:float, Pplantoutlet:float, PumpingPowerProd):

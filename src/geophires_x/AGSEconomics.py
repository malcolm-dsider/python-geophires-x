...
class AGSEconomics(Economics.Economics, AdvGeoPHIRESUtils.AdvGeoPHIRESUtils):
    """
    AGSEconomics Child class of Economics; it is the same, but has advanced AGS closed-loop functionality

    Args:
        Economics (Economics): The parent class
    ...
    def read_parameters(self, model:AdvModel) -> None:
        ...
        """
        model.logger.info("Init " + str(__class__) + ": " + sys._getframe().f_code.co_name)
        super().read_parameters(model) #read the default parmeters
    ...
    def verify(self, model:AdvModel) -> int:
        ...
        if model.surfaceplant.End_use not in (EndUseOptions.HEAT, EndUseOptions.ELECTRICITY):
            print("Error: CLGS model database imposes additional range restrictions: Economic Calculations can only be made only for electrcity or heat, not a combination.")
            model.logger.fatal("Error: CLGS model database imposes additional range restrictions:  Economic Calculations can only be made only for electrcity or heat, not a combination.")
            self.error = 1
        ...
        if self.econmodel.value != EconomicModel.CLGS: #do a classical econ calculation
           super().Calculate(model)
            ...
            if model.surfaceplant.End_use == EndUseOptions.HEAT:
                self.CAPEX_Surface_Plant = np.max(model.surfaceplant.Instantaneous_heat_production)*self.Direct_use_heat_cost_per_kWth.value/1e6 #[M$]
            ...
            if model.surfaceplant.End_use == EndUseOptions.HEAT:
                ...
                self.LCOH.value = (self.TotalCAPEX + np.sum(self.OPEX_Plant*Discount_vector))*1e6/np.sum(model.surfaceplant.Annual_heat_production/1e3*Discount_vector) #$/MWh
                ...
                if model.surfaceplant.Average_electricity_production == 0:
                    ...
                    self.LCOE.value = (self.TotalCAPEX + np.sum(self.OPEX_Plant*Discount_vector))*1e6/np.sum((model.surfaceplant.Annual_electricity_production-model.surfaceplant.Annual_pumping_power)/1e3*Discount_vector) #$/MWh
            ...
            if len(model.surfaceplant.error_codes) > 0:
                model.logger.fatal("failed with the following error codes: " + str(model.surfaceplant.error_codes[0:]) +" in "+ str(__class__) + " " + os.path.abspath(__file__))
                print("Error: failed with the following error codes" + str(model.surfaceplant.error_codes[0:]) +" in "+ str(__class__) + " " + os.path.abspath(__file__) +".  Exiting....")
                sys.exit()

            #copy values to GEOPHIRES Equivilents

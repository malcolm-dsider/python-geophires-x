...
from geophires_x.Parameter import intParameter, floatParameter, OutputParameter, ReadParameter, ConvertUnitsBack, \
...
class HIP_RA():
    ...
    T = [0.01, 10.0, 20.0, 25.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100.0, 110.0, 120.0, 140.0, 160.0, 180.0,
         200.0, 220.0, 240.0, 260.0, 280.0, 300.0, 320.0, 340.0, 360.0, 373.946]
    EntropyH20 = [0.0, 0.15109, 0.29648, 0.36722, 0.43675, 0.5724, 0.70381, 0.83129, 0.95513, 1.0756, 1.1929, 1.3072,
                  1.4188, 1.5279, 1.7392, 1.9426, 2.1392, 2.3305, 2.5177, 2.702, 2.8849, 3.0685, 3.2552, 3.4494, 3.6601,
                  3.9167, 4.407]
    EnthalpyH20 = [0.000612, 42.021, 83.914, 104.83, 125.73, 167.53, 209.34, 251.18, 293.07, 335.01, 377.04, 419.17,
                   461.42, 503.81, 589.16, 675.47, 763.05, 852.27, 943.58, 1037.6, 1135.0, 1236.9, 1345.0, 1462.2,
                   1594.5, 1761.7, 2084.3]
    ...
    def __init__(self):
        ...
        self.ReservoirTemperature = self.ParameterDict[self.ReservoirTemperature.Name] = floatParameter(
            "Reservoir Temperature", value=150.0, Min=50, Max=1000, UnitType=Units.TEMPERATURE,
            PreferredUnits=TemperatureUnit.CELCIUS, CurrentUnits=TemperatureUnit.CELCIUS, Required=True,
        ...
        self.RejectionTemperature = self.ParameterDict[self.RejectionTemperature.Name] = floatParameter(
            "Rejection Temperature", value=25.0, Min=0.1, Max=200, UnitType=Units.TEMPERATURE,
            PreferredUnits=TemperatureUnit.CELCIUS, CurrentUnits=TemperatureUnit.CELCIUS, Required=True,
            ErrMessage="assume default rejection temperature (25 deg-C)",
            ToolTipText="Rejection Temperature [25 dec-C]")
        self.FormationPorosity = self.ParameterDict[self.FormationPorosity.Name] = floatParameter("Formation Porosity",
                                                                                                  value=18.0, Min=0.0,
                                                                                                  Max=100.0,
        ...
        self.ReservoirArea = self.ParameterDict[self.ReservoirArea.Name] = floatParameter("Reservoir Area", value=81.0,
                                                                                          Min=0.0, Max=10000.0,
                                                                                          UnitType=Units.AREA,
        ...
        self.ReservoirThickness = self.ParameterDict[self.ReservoirThickness.Name] = floatParameter(
            "Reservoir Thickness", value=0.286, Min=0.0, Max=10000.0, UnitType=Units.LENGTH,
            PreferredUnits=LengthUnit.KILOMETERS, CurrentUnits=LengthUnit.KILOMETERS, Required=True,
            ErrMessage="assume default reservoir thickness (0.286 km2)", ToolTipText="Reservoir Thickness [0.286 km]")
        self.ReservoirLifeCycle = self.ParameterDict[self.ReservoirLifeCycle.Name] = intParameter(
            "Reservoir Life Cycle", value=30, UnitType=Units.TIME, PreferredUnits=TimeUnit.YEAR,
            CurrentUnits=TimeUnit.YEAR, AllowableRange=list(range(1, 101, 1)), Required=True,
            ErrMessage="assume default Reservoir Life Cycle (25 years)", ToolTipText="Reservoir Life Cycle [30 years]")
        ...
        self.ReservoirHeatCapacity = self.ParameterDict[self.ReservoirHeatCapacity.Name] = floatParameter(
            "Reservoir Heat Capacity", value=2.84E+12, Min=0.0, Max=1E+14, UnitType=Units.HEAT_CAPACITY,
            PreferredUnits=HeatCapacityUnit.KJPERKM3C, CurrentUnits=HeatCapacityUnit.KJPERKM3C, Required=True,
        ...
        self.HeatCapacityOfWater = self.ParameterDict[self.HeatCapacityOfWater.Name] = floatParameter(
            "Heat Capacity Of Water", value=-1.0, Min=3.0, Max=10.0, UnitType=Units.HEAT_CAPACITY,
            PreferredUnits=HeatCapacityUnit.kJPERKGC, CurrentUnits=HeatCapacityUnit.kJPERKGC, Required=True,
        ...
        self.HeatCapacityOfRock = self.ParameterDict[self.HeatCapacityOfRock.Name] = floatParameter(
            "Heat Capacity Of Rock", value=1.000, Min=0.0, Max=10.0, UnitType=Units.HEAT_CAPACITY,
            PreferredUnits=HeatCapacityUnit.kJPERKGC, CurrentUnits=HeatCapacityUnit.kJPERKGC, Required=True,
            ErrMessage="assume default Heat Capacity Of Rock (1.0 kJ/kgC)",
            ToolTipText="Heat Capacity Of Rock [1.0 kJ/kgC]")
        self.DensityOfWater = self.ParameterDict[self.DensityOfWater.Name] = floatParameter("Density Of Water",
                                                                                            value=-1.0, Min=1.000E+11,
                                                                                            Max=1.000E+13,
        ...
        self.DensityOfRock = self.ParameterDict[self.DensityOfRock.Name] = floatParameter("Density Of Rock",
                                                                                          value=2.55E+12, Min=1.000E+11,
                                                                                          Max=1.000E+13,
                                                                                          UnitType=Units.DENSITY,
                                                                                          PreferredUnits=DensityUnit.KGPERKILOMETERS3,
                                                                                          CurrentUnits=DensityUnit.KGPERKILOMETERS3,
        ...
        self.RecoverableHeat = self.ParameterDict[self.RecoverableHeat.Name] = floatParameter("Recoverable Heat",
                                                                                              value=-1.0, Min=0.001,
                                                                                              Max=1.000,
                                                                                              UnitType=Units.PERCENT,
                                                                                              ...
                                                                                              Required=False,
                                                                                              ErrMessage="assume 0.66 for high-T reservoirs (>150C), 0.43 for low-T reservoirs (>90, Garg and Combs (2011)",
                                                                                              ToolTipText="percent of heat that is recoverable from the rock in the reservoir 0.66 for high-T reservoirs, 0.43 for low-T reservoirs (Garg and Combs (2011)")

        # internal
        self.WaterContent = self.ParameterDict[self.WaterContent.Name] = floatParameter("Water Content", value=18.0,
                                                                                        Min=0.0, Max=100.0,
                                                                                        UnitType=Units.PERCENT,
                                                                                        PreferredUnits=PercentUnit.PERCENT,
                                                                                        CurrentUnits=PercentUnit.PERCENT,
        ...
        self.RockContent = self.ParameterDict[self.RockContent.Name] = floatParameter("Rock Content", value=82.0,
                                                                                      Min=0.0, Max=100.0,
                                                                                      UnitType=Units.PERCENT,
                                                                                      PreferredUnits=PercentUnit.PERCENT,
                                                                                      CurrentUnits=PercentUnit.PERCENT,
        ...
        self.RejectionTemperatureK = self.ParameterDict[self.RejectionTemperatureK.Name] = floatParameter(
            "Rejection Temperature in K", value=298.15, Min=0.1, Max=1000.0, UnitType=Units.TEMPERATURE,
            PreferredUnits=TemperatureUnit.KELVIN, CurrentUnits=TemperatureUnit.KELVIN, Required=True,
            ErrMessage="assume default rejection temperature in K (298.15 deg-K)",
            ToolTipText="Rejection Temperature in K [298.15 deg-K]")
        self.RejectionEntropy = self.ParameterDict[self.RejectionEntropy.Name] = floatParameter("Rejection Entropy",
                                                                                                value=0.3670,
        ...
        self.RejectionEnthalpy = self.ParameterDict[self.RejectionEnthalpy.Name] = floatParameter("Rejection Enthalpy",
                                                                                                  value=104.8,
                                                                                                  Min=0.0001,
                                                                                                  Max=1000.0,
        ...
        self.V = self.OutputParameterDict[self.V.Name] = OutputParameter(Name="Reservoir Volume", value=-999.9,
                                                                         UnitType=Units.VOLUME,
                                                                         PreferredUnits=VolumeUnit.KILOMETERS3,
                                                                         CurrentUnits=VolumeUnit.KILOMETERS3)
        self.qR = self.OutputParameterDict[self.qR.Name] = OutputParameter(Name="Stored Heat", value=-999.9,
                                                                           UnitType=Units.HEAT,
                                                                           PreferredUnits=HeatUnit.KJ,
                                                                           CurrentUnits=HeatUnit.KJ)
        self.mWH = self.OutputParameterDict[self.mWH.Name] = OutputParameter(Name="Fluid Produced", value=-999.9,
                                                                             UnitType=Units.MASS,
                                                                             PreferredUnits=MassUnit.KILOGRAM,
                                                                             CurrentUnits=MassUnit.KILOGRAM)
        self.e = self.OutputParameterDict[self.e.Name] = OutputParameter(Name="Enthalpy", value=-999.9,
                                                                         UnitType=Units.ENTHALPY,
                                                                         PreferredUnits=EnthalpyUnit.KJPERKG,
                                                                         CurrentUnits=EnthalpyUnit.KJPERKG)
        self.qWH = self.OutputParameterDict[self.qWH.Name] = OutputParameter(Name="Wellhead Heat", value=-999.9,
                                                                             UnitType=Units.HEAT,
                                                                             PreferredUnits=HeatUnit.KJ,
                                                                             CurrentUnits=HeatUnit.KJ)
        self.Rg = self.OutputParameterDict[self.Rg.Name] = OutputParameter(Name="Recovery Factor", value=-999.9,
                                                                           UnitType=Units.PERCENT,
                                                                           PreferredUnits=PercentUnit.PERCENT,
                                                                           CurrentUnits=PercentUnit.PERCENT)
        self.WA = self.OutputParameterDict[self.WA.Name] = OutputParameter(Name="Available Heat", value=-999.9,
                                                                           UnitType=Units.HEAT,
                                                                           PreferredUnits=HeatUnit.KJ,
                                                                           CurrentUnits=HeatUnit.KJ)
        self.WE = self.OutputParameterDict[self.WE.Name] = OutputParameter(Name="Produceable Heat", value=-999.9,
                                                                           UnitType=Units.HEAT,
                                                                           PreferredUnits=HeatUnit.KJ,
                                                                           CurrentUnits=HeatUnit.KJ)
        self.We = self.OutputParameterDict[self.We.Name] = OutputParameter(Name="Produceable Electricity", value=-999.9,
    ...
    def read_parameters(self) -> None:
            ...
            for item in self.ParameterDict.items():
                ...
                if key in self.InputParameters:
                    ...
                    ReadParameter(ParameterReadIn, ParameterToModify,
                                  self)  # this should handle all the non-special cases
                    ...
                    if ParameterToModify.Name == "Formation Porosity":
                        ...
                        if value < 0:  # if the user supplied -1 as the density, they want us to calculate it.
                            ParameterToModify.value = self.densitywater(
                                self.ReservoirTemperature.value) * 1_000_000_000.0
        ...
        for key in self.InputParameters.keys():
            if key.startswith("Units:"):
                ...
                self.OutputParameterDict[key.replace("Units:", "")].CurrentUnits = \
                LookupUnits(self.InputParameters[key].sValue)[0]
    ...
    def Calculate(self):
        ...
        if self.DensityOfWater.value < self.DensityOfWater.Min: self.DensityOfWater.value = self.densitywater(
            self.ReservoirTemperature.value) * 1_000_000_000.0
        if self.HeatCapacityOfWater.value < self.HeatCapacityOfWater.Min: self.HeatCapacityOfWater.value = self.heatcapacitywater(
            self.ReservoirTemperature.value) / 1000.0
        ...
        self.e.value = (self.EnthalpyH20_func(self.ReservoirTemperature.value) - self.RejectionEnthalpy.value) - (
                self.RejectionTemperatureK.value * (
                    self.EntropyH20_func(self.ReservoirTemperature.value) - self.RejectionEntropy.value))
        self.qWH.value = self.mWH.value * (
                self.EnthalpyH20_func(self.ReservoirTemperature.value) - self.RejectionTemperatureK.value)
        self.Rg.value = self.qWH.value / self.qR.value
        self.WA.value = self.mWH.value * self.e.value * self.Rg.value * self.recoverableheat(
            self.ReservoirTemperature.value)

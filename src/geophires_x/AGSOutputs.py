        ...
        try:
            if model.wellbores.IsAGS: #do a classical output display
                ...
                if len(model.wellbores.PumpingPower.value) != len(model.wellbores.ProducedTemperature.value):
                    f = scipy.interpolate.interp1d(np.arange(0, len(model.wellbores.PumpingPower.value)), model.wellbores.PumpingPower.value, fill_value="extrapolate")
                    model.wellbores.PumpingPower.value = f(np.arange(0, len(model.wellbores.ProducedTemperature.value), 1.0))   # use interpolation function returned by `interp1d`
                if len(model.surfaceplant.NetElectricityProduced.value) != len(model.wellbores.ProducedTemperature.value):
                    f = scipy.interpolate.interp1d(np.arange(0, len(model.surfaceplant.NetElectricityProduced.value)), model.surfaceplant.NetElectricityProduced.value, fill_value="extrapolate")
                ...
                if len(model.surfaceplant.FirstLawEfficiency.value) != len(model.wellbores.ProducedTemperature.value):
                    f = scipy.interpolate.interp1d(np.arange(0, len(model.surfaceplant.FirstLawEfficiency.value)), model.surfaceplant.FirstLawEfficiency.value, fill_value="extrapolate")
            ...
            if not model.economics.econmodel.value == EconomicModel.CLGS:
                ...
                with open(outputfile,'w', encoding='UTF-8') as f:
                    ...
                    f.write("      Vertical Depth:                                      " + str(round(model.reserv.InputDepth.value)) + " " + model.reserv.InputDepth.CurrentUnits.value + NL)
                    f.write(f"      Geothermal Gradient:                                   " + "{0:.4f}".format((model.reserv.gradient.value[0])) +" " + model.reserv.gradient.CurrentUnits.value + NL)
                    f.write(f"      Wellbore Diameter:                                      " + "{0:.4f}".format((model.wellbores.prodwelldiam.value)) + " " + model.wellbores.prodwelldiam.CurrentUnits.value + NL)
                    f.write(f"      Injection Temperature:                                 " + "{0:.1f}".format((model.wellbores.Tinj.value)) +" " + TemperatureUnit.CELCIUS.value + NL)
                    f.write(f"      Thermal Conductivity:                                   " + "{0:.2f}".format((model.reserv.krock.value)) +" " +model.reserv.krock.CurrentUnits.value + NL)

                    f.write(" ")
                    f.write('### Reservoir Simulation Results ###' + NL)
                    f.write(f"      Average Production Temperature:                       " + "{0:.1f}".format((model.surfaceplant.AveProductionTemperature.value)) + " " + model.surfaceplant.AveProductionTemperature.CurrentUnits.value + NL)
                    f.write(f"      Average Production Pressure:                          " + "{0:.1f}".format((model.surfaceplant.AveProductionPressure.value)) + " " + model.surfaceplant.AveProductionPressure.CurrentUnits.value + NL)
                    f.write(f"      Average Heat Production:                             " + "{0:.1f}".format((model.surfaceplant.AveInstHeatProduction.value)) +" " + model.surfaceplant.AveInstHeatProduction.CurrentUnits.value + NL)
                    f.write(f"      First Year Heat Production:                         " + "{0:.1f}".format((model.surfaceplant.FirstYearHeatProduction.value/1e3)) + " " + model.surfaceplant.FirstYearHeatProduction.CurrentUnits.value + NL)

                    #Print results for heating or electricty
                    if model.surfaceplant.End_use==EndUseOptions.ELECTRICITY:
                        f.write(f"      Average Net Electricity Production:                   " + "{0:.1f}".format((model.surfaceplant.AveInstNetElectricityProduction.value)) +" " + model.surfaceplant.AveInstNetElectricityProduction.CurrentUnits.value + NL)
                        f.write(f"      First Year Electricity Production:                   " + "{0:.1f}".format((model.surfaceplant.FirstYearElectricityProduction.value/1e3)) + " " + model.surfaceplant.FirstYearElectricityProduction.CurrentUnits.value + NL)

                    f.write(" ")
                    f.write('### Cost Results ###' + NL)
                    f.write(f"      Total CAPEX:                                           " + "{0:.1f}".format((model.economics.CCap.value)) + " " + model.economics.CCap.CurrentUnits.value + NL)
                    f.write(f"      Drilling Cost:                                         " + "{0:.1f}".format((model.economics.Cwell.value)) + " " + model.economics.Cwell.CurrentUnits.value + NL)
                    f.write(f"      Surface Plant Cost:                                     " + "{0:.1f}".format((model.economics.Cplant.value)) + " " + model.economics.Cplant.CurrentUnits.value + NL)
                    f.write(f"      OPEX:                                                  " + "{0:.1f}".format((model.economics.Coam.value)) + " " + model.economics.Coam.CurrentUnits.value + NL)
                    ...
                    if model.surfaceplant.enduseoption.value == EndUseOptions.ELECTRICITY:  #only electricity
                        ...
                        f.write('  YEAR               HEAT                       HEAT                RESERVOIR           CUM PERCENTAGE OF\n')
                        f.write('                    PROVIDED                   EXTRACTED            HEAT CONTENT        TOTAL HEAT MINED\n')
                        f.write('                   (GWh/year)                  (GWh/year)            (10^15 J)                 (%)\n')
                        for i in range(0, model.surfaceplant.plantlifetime.value-1):
                            f.write('  {0:2.0f}              {1:8.1f}                    {2:8.1f}              {3:8.2f}               {4:8.2f}'.format(i+1,
